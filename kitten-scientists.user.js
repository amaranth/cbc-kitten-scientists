...
var run = function() {
    ...
    if (game.colorScheme !== 'sleek') {
        left.css('line-height', '95%');
        middle.css('width', '680px');
        right.css('width', '350px');

        addRule('.toolbarIcon {'
            + 'font-size: 16px !important;'
            + '}');

        addRule('.scheme_dark .maxRes {'
            + 'color: #999 !important;'
            + '}');
    }

    addRule('.resourceRow td {'
    ...
    document.addEventListener("DOMNodeInserted", function(event) {
        ...
        if (target.is('.tabInner > div > a') || target.is('.tabInner > div > span:contains("|")')) {
            target.hide();
        }
    });

    addRule('.bldGroupContainer {'
        + 'padding-top: 0 !important;'
